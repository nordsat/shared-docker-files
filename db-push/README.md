...
This folder contains the scripts used to sync files into a POSTGIS database.
...
#### for ope
```
podman run -d --replace  --name db-sync --network pytroll_network -v /eodata/hrit_out/:/mnt/output/ -v ./output:/usr/local/bin/db-sync/output/ -v ./db-sync.yaml:/usr/local/bin/db-sync/db-sync.yaml -v ./trollflow2.yaml:/usr/local/bin/db-sync/trollflow2.yaml db-sync python3 db_sync.py db-sync.yaml trollflow2.yaml
```
